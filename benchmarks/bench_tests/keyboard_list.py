...
if __name__ == "__main__":
    ...
    for flags in itertools.product([True, False], repeat=3):
        ...
        for num_thread in (1, 2, 4, 8):
            bench_result[flags][num_thread] = {}
            my_bench = timeit.repeat(
                partial(my_lib_aug.augment, data=REVIEWS, num_thread=num_thread),
                repeat=REPEATS,
                number=NUMBER_MY_LIB,
            )
            bench_result[flags][num_thread]["fasttextaug"] = [i / NUMBER_MY_LIB for i in my_bench]
            bench_result[flags][num_thread]["fasttextaug_number"] = NUMBER_MY_LIB
            bench_result[flags][num_thread]["fasttextaug_repeats"] = REPEATS
            ...
            if num_thread == 1:
                ...
                bench_result[flags][num_thread]["nlpaug"] = [
                    i / NUMBER_THEIR_LIB for i in lib_bench
                ]
                bench_result[flags][num_thread]["nlpaug_number"] = NUMBER_THEIR_LIB
                bench_result[flags][num_thread]["nlpaug_repeats"] = REPEATS
